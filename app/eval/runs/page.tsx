...
async function loadReports() {
  return listCourseDiagnosticReports();
}
...
EVAL_USE_MOCK_LLM=true npm run eval:course -- --input examples/eval/sample-course.txt --topic "人工智能的三大基石" --targetLearner "大学一年级非计算机专业学生" --lessonIndex 1 --expectedDurationMinutes 10 --sourceSystem "OpenMAIC"`}
        ...
        <table style={{ borderCollapse: 'collapse', minWidth: 1200 }}>
          <thead>
            <tr>
              {['runId', 'topic', 'sourceSystem', 'targetLearner', 'lessonIndex', 'expectedDurationMinutes', 'estimatedDurationMinutes', 'scaffoldStabilityScore', 'interactionEffectivenessScore', 'teachingEfficiencyScore', 'overallScore', 'createdAt'].map((header) => (
          ...
          <tbody>
            {reports.map((report) => (
              <tr key={report.runId}>
                ...
                <td style={{ border: '1px solid #ddd', padding: 8 }}>{report.sourceSystem}</td>
